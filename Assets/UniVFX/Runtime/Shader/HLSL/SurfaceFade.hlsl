...
#define SurfaceFadeCalc(worldPos, viewPos, viewDir, normal, sceneDepth)\
    float surfaceFade = 1; \
    float surfaceFadeOut = GetVertexDataArray1(_SurfaceFadeOut); \
    float surfaceFadeIn = GetVertexDataArray1(_SurfaceFadeIn); \
    if (_Frenel)\
    {\
        float rimFade = saturate(dot(normal, viewDir)); \
        float frenelPow = GetVertexDataArray1(_FrenelPow); \
